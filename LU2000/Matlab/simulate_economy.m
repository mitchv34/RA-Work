function [sim_results] = simulate_economy(params, ss, sigma, T, T_burn)
    ...
    C_sp_star = zeros(T, 1);        % Consumption optimal taxation;
    C_sp_bar  = zeros(T, 1);        % Consumption fixed taxation (steady state level);
    C_ls      = zeros(T, 1);        % Consumption laissez faire (no taxation);
    X_sp_bar  = zeros(T, 1);        % Measure of past average consumption associated with C_sp_bar;
    X_sp_star = zeros(T, 1);        % Measure of past average consumption associated with C_sp_star;
    X_ls      = zeros(T, 1);        % Measure of past average consumption associated with C_ls;
    n_sp_star = zeros(T, 1);        % Labor supply under optimal taxation;
    n_sp_bar  = zeros(T, 1);        % Labor supply under fixed taxation;
    n_ls      = zeros(T, 1);        % Labor supply under laissez faire;
