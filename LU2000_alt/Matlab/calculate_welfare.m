function [Delta] = calculate_welfare(stochastic_economy, det_economy, params)
    ...
    U_sp_star     = mean( ( ((C_sp_star).^(1 - gamma) - 1) / (1 - gamma) - xi * X_sp_star - A * n_sp_star)' * beta_t);
    U_sp_bar      = mean( ( ((C_sp_bar).^(1 - gamma) - 1)  / (1 - gamma) - xi * X_sp_bar  - A * n_sp_bar)'  * beta_t);
    U_ls          = mean( ( ((C_ls).^(1 - gamma) - 1)      / (1 - gamma) - xi * X_ls      - A * n_ls)'      * beta_t);
    ...
    % Create equations
    eq_1          = @(delta) ((((1-delta) .* C_sp ).^(1-gamma)-1)/(1-gamma) - xi * X_sp - A * n_sp)*beta_t - U_sp_star;
    eq_2          = @(delta) ((((1-delta) .* C_sp ).^(1-gamma)-1)/(1-gamma) - xi * X_sp - A * n_sp)*beta_t - U_sp_bar;
    eq_3          = @(delta) ((((1-delta) .* C_sp ).^(1-gamma)-1)/(1-gamma) - xi * X_sp - A * n_sp)*beta_t - U_ls;
    eq_4          = @(delta) ((((1-delta) .* C_ls ).^(1-gamma)-1)/(1-gamma) - xi * X_ls - A * n_ls)*beta_t - U_ls;
