...
%     n_ls      = stochastic_economy(9*Nsim+1:10*Nsim,:);
    ...
    U_sp_star = mean( sum( ( ((C_sp_star).^(1 - gamma) - 1) / (1 - gamma) - xi * X_sp_star - A * n_sp_star) .* beta_t) );
    U_sp_bar  = mean( sum( ( ((C_sp_bar).^(1 - gamma) - 1)  / (1 - gamma) - xi * X_sp_bar  - A * n_sp_bar)  .* beta_t) );
    U_ls      = mean( sum( ( ((C_ls).^(1 - gamma) - 1)      / (1 - gamma) - xi * X_ls      - A * n_ls)      .* beta_t) );
